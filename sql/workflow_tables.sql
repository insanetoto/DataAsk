...
CREATE TABLE IF NOT EXISTS `sys_workflow` (
  ...
  `retry_count` int(11) DEFAULT 1 COMMENT '重试次数',
...
CREATE TABLE IF NOT EXISTS `workflow_steps` (
  ...
  `step_order` int(11) NOT NULL COMMENT '执行顺序',
  `depends_on` json COMMENT '依赖的步骤ID列表',
  `config` json NOT NULL COMMENT '步骤配置',
  `timeout` int(11) DEFAULT 1800 COMMENT '步骤超时时间(秒)',
  `retry_count` int(11) DEFAULT 1 COMMENT '重试次数',
...
CREATE TABLE IF NOT EXISTS `workflow_executions` (
  ...
  `executor_id` int(11) COMMENT '执行人ID',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `workflow_step_executions` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '步骤执行记录ID',
  `execution_id` int(11) NOT NULL COMMENT '工作流执行记录ID',
...
-- ('工作流管理', 'workflow:manage', 'menu', '工作流管理权限'),
-- ('工作流查看', 'workflow:view', 'button', '查看工作流权限'),
-- ('工作流创建', 'workflow:create', 'button', '创建工作流权限'),
-- ('工作流编辑', 'workflow:edit', 'button', '编辑工作流权限'),
-- ('工作流删除', 'workflow:delete', 'button', '删除工作流权限'),
-- ('工作流执行', 'workflow:execute', 'button', '执行工作流权限'); 