...
class EnhancedWorkflowService:
    ...
    def get_workspace_categories(self, workspace_id: int, user_id: int) -> Dict:
        """获取工作域下的业务流程分类"""
        try:
            # 检查权限
            if not self._check_workspace_permission(workspace_id, user_id, 'view'):
                return {
                    'success': False,
                    'message': '您没有权限访问此工作域'
                }
            ...
            with self.main_engine.begin() as conn:
                sql = text("""
                    SELECT c.*, 
                           COUNT(wf.id) as workflow_count,
                           COUNT(sub_c.id) as subcategory_count
                    FROM workflow_categories c
                    LEFT JOIN enhanced_workflows wf ON wf.category_id = c.id AND wf.status IN ('active', 'inactive')
                    LEFT JOIN workflow_categories sub_c ON sub_c.parent_id = c.id AND sub_c.status = 'active'
                    WHERE c.workspace_id = :workspace_id AND c.status = 'active'
                    GROUP BY c.id
                    ORDER BY c.order_num, c.name
                """)
                
                result = conn.execute(sql, {'workspace_id': workspace_id})
                categories = []
                ...
                for row in result:
                    categories.append({
                        ...
                        'order_num': row.order_num,
                        'workflow_count': row.workflow_count or 0,
                        'subcategory_count': row.subcategory_count or 0,
                        'created_at': row.created_at.isoformat() if row.created_at else None
                    })
            ...
            with self.main_engine.begin() as conn:
                ...
                if 'nodes' in workflow_data:
                    for node_data in workflow_data['nodes']:
                        self._create_workflow_node(conn, workflow_id, node_data, user_id)
                
                # 如果提供了连接配置，创建连接
                if 'connections' in workflow_data:
                    for connection_data in workflow_data['connections']:
                        self._create_node_connection(conn, workflow_id, connection_data)
            ...
            with self.main_engine.begin() as conn:
                ...
                for row in result:
                    ...
                    nodes.append({
                        ...
                        'rollback_config': json.loads(row.rollback_config) if row.rollback_config else None,
                ...
                connections_sql = text("""
                    SELECT c.*, 
                           n1.name as from_node_name, n1.code as from_node_code,
                           n2.name as to_node_name, n2.code as to_node_code
                    FROM workflow_node_connections c
                    LEFT JOIN workflow_nodes n1 ON n1.id = c.from_node_id
                    LEFT JOIN workflow_nodes n2 ON n2.id = c.to_node_id
    ...
    def _create_workflow_node(self, conn, workflow_id: int, node_data: Dict, user_id: int) -> int:
        ...
        insert_sql = text("""
            INSERT INTO workflow_nodes (
                ...
                x_position, y_position, icon, color, config, input_schema, output_schema,
                validation_rules, timeout_minutes, retry_count, retry_delay_minutes,
                skip_on_failure, rollback_enabled, rollback_config, conditions, creator_id
            ) VALUES (
                :workflow_id, :name, :code, :description, :type, :subtype, :step_order,
                :x_position, :y_position, :icon, :color, :config, :input_schema, :output_schema,
                :validation_rules, :timeout_minutes, :retry_count, :retry_delay_minutes,
        ...
        result = conn.execute(insert_sql, {
            ...
            'input_schema': json.dumps(node_data.get('input_schema')) if node_data.get('input_schema') else None,
            'output_schema': json.dumps(node_data.get('output_schema')) if node_data.get('output_schema') else None,
            'validation_rules': json.dumps(node_data.get('validation_rules')) if node_data.get('validation_rules') else None,
    ...
    def _create_node_connection(self, conn, workflow_id: int, connection_data: Dict) -> int:
        """创建节点连接的内部方法"""
        insert_sql = text("""
            INSERT INTO workflow_node_connections (
                workflow_id, from_node_id, to_node_id, condition_type,
                condition_expression, condition_config, order_num, style_config
            ) VALUES (
                :workflow_id, :from_node_id, :to_node_id, :condition_type,
                :condition_expression, :condition_config, :order_num, :style_config
            )
        ...
        result = conn.execute(insert_sql, {
            ...
            'condition_type': connection_data.get('condition_type', 'always'),
            'condition_expression': connection_data.get('condition_expression'),
            'condition_config': json.dumps(connection_data.get('condition_config')) if connection_data.get('condition_config') else None,
            'order_num': connection_data.get('order_num', 0),
            'style_config': json.dumps(connection_data.get('style_config')) if connection_data.get('style_config') else None
        })
...
# 服务实例化函数
def get_enhanced_workflow_service() -> EnhancedWorkflowService:
    """获取增强版工作流服务实例"""
